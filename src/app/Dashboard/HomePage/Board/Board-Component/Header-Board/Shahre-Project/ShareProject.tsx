...
const ShareProject: React.FC<ShareWorkspaceProp> = (): JSX.Element => {
    const [shareProject, setShareProject] = useState('')
const [CloseShareProject,setCloseShareProject] = useState(false)

    const [showPermission,setShowPermission]=useState(false)

return (
        <div className={`${CloseShareProject ? 'hidden': ''}`}>

            <Form style={customStyle}>
                {/* ---card body ------START*/}
                {/*--------اشتراک گذاری------------*/}
                <div className="flex justify-center items-center w-[430px] h-[33px] gap-[40px] ">
                    <div className="w-[452px] h-[32px] flex flex-row items-center justify-between gap-[10px]">
                        <div className="w-[24px] h-[24px]">
                            {/*<FontAwesomeIcon icon={faArrowLeft}/>*/}
                        </div>
                        <div className="font-extrabold
                 text-[24px] leading-[32px] text-center text-[#1E1E1E]">به اشتراک گذاری پروژه
                        </div>

                        <span onClick={()=>setCloseShareProject(!CloseShareProject)} className=" cursor-pointer text-[#323232] w-[24px]
                        h-[24px] ml-4"><FontAwesomeIcon
                            icon={faTimes}/>
                        </span>
                ...
                {/*------دعوت با ایمیل-----*/}
                <div dir="rtl" className="w-[430px] gap-[24px] h-[220px] ">
                <span className=" w-[430px] h-[40px]  flex">
                    <DynamicInput
                        width={339}
                        height={40}
                        value={shareProject}
                    ...
                    {/*    --------------لینک خصوصی---------*/}
                    <div dir="rtl" className="mt-4 w-[430px]  h-[22px] flex justify-between items-center">
                    ...
                    {/*-----------------اشتراک گذاشته شده-----------*/}
                    <div dir="rtl" className="mt-4 w-[430px] h-[110px] gap-[16px] ">
            <span className="w-[131px] h-[20px] font-medium text-[14px] text-[#7D828C]
             leading-[19.73px] text-center">اشتراک گذاشته شده با</span>
                        <div className="w-[507px] h-[115px] gap-[8px] ">
                            {/*----------Workspace Owner---------*/}
                            <div className="w-[430px] h-[33px] flex justify-between items-center ">
                   ...
                   text-[14px] leading-[19.73px] text-right">من</span>
                      ...
                      font-medium text-[12px] leading-[16.91px] text-[#A6A7A7] text-right">دسترسی کامل
                            ...
                            {/*    --------------------@Gmail --section--1-----------*/}
                            <div className=" w-[430px] h-[33px] flex justify-between items-center mt-4 ">
                                ...
                                {/*-------دسترسی کامل--------*/}
                                <div className="w-[117px] h-[27px] gap-[10px] pt-[5px]
                                pr-[8px] pb-[4px] pl-[8px] flex items-center">
                                    <div className={`${!showPermission ? 'hidden' : 'visible'}`}>
                                        <Permission/>
                                    </div>

                                    <select onClick={() => setShowPermission(!showPermission)}
                                             className=" outline-0 rounded-[6px] border-2 border-[#E9EBF0]">
                                        <option
                                            className="font-[400] text-[12px]  text-right leading-[16.91px]">
                                            دسترسی کامل
                                        </option>
